...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    vec2 mo = vec2(0);//iMouse.xy/u_resolution.xy;
    float time = .0; //u_time;

    // camera	
    vec3 ro = vec3(.0, -sin(u_time) / 15., -0.0);//vec3( 4.6*cos(0.1*time + 6.0*mo.x), 1.0 + 2.0*mo.y, 0.5 + 4.6*sin(0.1*time + 6.0*mo.x) );
    ...
    tot = vec3(voronoi(p.xy*8., u_time));

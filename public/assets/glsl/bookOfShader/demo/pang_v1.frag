...
float shoesDist(vec3 p) {
    ...
    p -= vec3(-.55, -.8, 0.4);
    p.x = -p.x;
    p.xz = rotmat(cos(u_time * 3.0) * 0.2) * p.xz;
...
float sceneDist(vec3 p) {
    ...
    // left arm
     op = p;
    p -= vec3(.66, .7, 0);
    p.xz = rotmat(cos(u_time * 3.0) * 0.2) * p.xz; // 左臂摆动
    ...
    op = p;
    p -= vec3(-.75, 0.2, 0);
    p.xz = rotmat(-cos(u_time * 3.0) * 0.2) * p.xz; // 右臂摆动
    d = smin(d, (length(p * vec3(1, 1.5, 1)) - .54), .03);
    p = op;

    // mouth
    p.y -= sin(u_time * 6.0)*.11;

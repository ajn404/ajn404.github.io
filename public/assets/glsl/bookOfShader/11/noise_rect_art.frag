...
void main() {
    ...
    float u_borderWidth = 4.;   // Constant border width
    vec4 color = vec4(noise(vec2(abs(sin(u_time / 100.)) * 2.)), .2, 1., 1.0); // Red border color
 ...
 // 定义右上角矩形区域的位置和大小
    ...
    // vec4 textureColor = texture2D(u_texture0, st);

    // Combine the border and texture colors
